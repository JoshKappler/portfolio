...
- The letter is Press Roman, every element, with size as the only
  variation; Courier New appears only in the /resume viewer chrome and
  the globe's canvas glyphs. Press Roman is our own worn cut of Gelasio
  (SIL OFL, license in public/fonts/OFL.txt), self-hosted in
  public/fonts/, baked by `scripts/generate-press-roman.py` (run with a
  venv holding fonttools + brotli, passing the Gelasio variable TTF):
  seeded noise bends every outline (warble) and roughens its edges
  (wear), so the press character lives in the vectors and every engine
  prints it identically. Gelasio shares Georgia's advance widths, so
  copy tuned for justified squares keeps its wraps, and Georgia is the
  metric-identical fallback.
...
- Ink character ships baked into assets, never as live SVG filters on
  text: engines rasterize turbulence and displacement very differently
  (Firefox faint, WebKit heavy), which once made desktop and mobile
  render as two different sites. Any change to the site's look must be
  screenshot-verified in BOTH headless Firefox and headless Chrome.
- `src/components/globe/mark-geometry.js` and `public/jk-mark.png` are
  ...
  system Georgia.ttf by file). `public/jk-mark-inked.png` is that sprite
  printed through the mark's ink filter and tinted, generated by
  `scripts/generate-inked-mark.html` (run with --allow-file-access-from-files,
  then oxipng): the globe server-renders it as a still so the mark is in the
  first paint, and the canvas hides it on its first frame. Regenerate all
  three together; never hand-edit.
- `src/app/apple-icon.png` is the mark printed as ink on a parchment tile
  for iOS home screens (opaque square; iOS masks its own corners).
  Regenerate it from `public/jk-mark.png` whenever the mark changes.
- There are no visual dependencies. Do not add animation, background, or
  font packages; the plainness is the design. The only fonts served are
  the two self-hosted Press Roman files; nothing loads from a third
  party.
...
- The footer colophon names Press Roman and claims no trackers. Adding
  analytics, any third-party script, or a font served off-site makes it
  false; update or remove the line in the same change.
